...
#      WorkOS credentials.
#   2. apps/app/.env.development.local must exist + be filled. Copy
...
# shell or apps/app/.env.development.local (electron-vite inlines
# the file at dev build time; shell exports take precedence).
app_env=apps/app/.env.development.local
has_app_var() {
...
if ! has_app_var SPOOL_WORKOS_CLIENT_ID; then
  ...
  echo "       Fill it in apps/app/.env.development.local (copy the" >&2
