...
describe('materialize → share lineage round-trip', () => {
  ...
  it('materializes with rewritten sessionId, restored paths, and one birth record', async () => {
    ...
    expect(materialized.resumeArgv).toEqual([
      'claude',
      '--resume',
      'new-session-id',
      '--fork-session',
  ...
  it('materializes a codex session with rewritten session_meta id and one birth record', async () => {
    const codexJsonl =
      [
        claudeLine({
          ...
          type: 'session_meta',
          payload: { id: 'orig-codex', session_id: 'orig-codex', cwd: WS },
        }),
        claudeLine({
          timestamp: '2026-07-16T10:00:01Z',
          type: 'turn_context',
          payload: { model: 'gpt-5-codex', cwd: WS },
        }),
        claudeLine({
          timestamp: '2026-07-16T10:00:02Z',
          type: 'event_msg',
          payload: { type: 'user_message', message: `rename alpha to beta in ${WS}/src/a.ts` },
        }),
        claudeLine({
          timestamp: '2026-07-16T10:00:03Z',
          type: 'event_msg',
          payload: { type: 'agent_message', message: 'Renamed alpha to beta.' },
        }),
      ].join('\n') + '\n'
    const prepared = await prepareShare({
      provider: 'codex',
    ...
    expect(materialized.fileName).toBe(
      'rollout-2026-07-16T12-34-56-aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee.jsonl',
    )
    expect(materialized.resumeArgv).toEqual([
      'codex',
      'fork',
    ...
    const meta = JSON.parse(materialized.lines[0] as string) as {
  ...
  it('extractBirthPayload ignores records without the marker', () => {
    expect(
      extractBirthPayload([claudeLine({ type: 'user', message: { role: 'user', content: 'hi' } })]),
    ).toBeNull()
  })
