...
`,
    ...
    expect(editNote('draft', { env: { ...process.env, EDITOR: editor } })).toBe(
      'Saved before the editor complained',
    )
  })
...
`,
    ...
    expect(() => editNote('draft', { env: { ...process.env, EDITOR: editor } })).toThrow(
      /was not modified — aborting/,
    )
  })
})
