...
export function buildSessionSummaryPrompt(
  ...
  const systemBody = [
    'Create a share-ready summary that explains both the purpose and the full journey of the selected Spool session below.',
    '',
    ...
    '- Write in the predominant language of the transcript, including headings and progress labels.',
    '',
    'Interpretation rules:',
    '- Treat the first user prompt as the clearest statement of the session’s original purpose. Introduce it prominently, while noting any later scope changes.',
    ...
    '',
    'Required structure:',
    '- Start with a one- or two-sentence overview anchored in the original purpose.',
    '- Use short, localized Markdown headings in this order: Purpose, Progress, Highlights, Result vs. original goal. Add Next steps only when the transcript supports them.',
    '- Under Progress, begin with a compact visual progress map using arrows, for example: `Goal → Investigation → Implementation → Validation → Result`. Adapt the stages and labels to what actually happened; do not use Mermaid or claim a stage that lacks evidence.',
    '- After the progress map, describe the major stages of the session in chronological order. Include meaningful errors, alternatives, reversals, and validation instead of listing only the final changes.',
    '- Under Highlights, surface the most notable moments. A highlight may be an unusually specific or insightful user prompt, a key discovery, a smart workaround, a consequential decision, an unexpected failure, or strong validation.',
    '- When the wording of a user prompt is itself a highlight, quote only a short, non-sensitive excerpt and explain why it mattered.',
    '- Under Result vs. original goal, explicitly classify the outcome as fully met, partially met, diverged, or unresolved, and justify that assessment with transcript evidence.',
