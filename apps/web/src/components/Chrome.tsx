...
// counterpart). session-view compiles its own Tailwind pass; dark: is
// bound to html[data-theme].
...
export function ThemeToggle() {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() =>
    typeof document === 'undefined' ? 'light' : readThemeAttr(),
  )
...
export function Header({ auth = 'auto' as AuthState }: { auth?: AuthState }) {
  ...
  const [resolved, setResolved] = useState<AuthState>(() => {
    ...
    // SSR/prerender renders the signed-out chrome; the cache read only
