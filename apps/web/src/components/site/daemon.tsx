...
function tokenize(line: string): { text: string; cls?: string | undefined }[] {
  const out: { text: string; cls?: string | undefined }[] = []
  let i = 0
  while (i < line.length) {
    const rest = line.slice(i)
    const cm = rest.match(/^\/\/[^\n]*/)
    if (cm) {
      out.push({ text: cm[0], cls: 'tok-cm' })
      i += cm[0].length
      continue
    }
    const s = rest.match(/^"[^"]*"|^'[^']*'|^`[^`]*`/)
    if (s) {
      out.push({ text: s[0], cls: 'tok-str' })
      i += s[0].length
      continue
    }
...
const LIFECYCLE = [
  {
    ...
    body: (
      <>
        Pick connectors from the registry — install via <code>spool-daemon install</code>, click an{' '}
  ...
  },
  {
    ...
    body: (
      <>
        Each connector declares its own schedule — hourly, daily, watched. The daemon respects rate
        limits, backs off on failure and resumes on next wake. Your laptop stays quiet.
      </>
    ),
    glyph: (
      <svg
  ...
  },
  {
    ...
    body: (
      <>
        Spotlight-style search in the app, or <code>spool-daemon search "…"</code> from the CLI.
        Full-text across everything that's been synced.
      </>
    ),
    glyph: (
      <svg
...
const PRINCIPLES = [
  {
    ...
    body: (
      <>
        One SQLite file at <code>~/.spool-daemon/spool-daemon.db</code>. No cloud sync, no
        analytics, no profile. Inspect it with <code>sqlite3</code> any time.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Every connector ships on npm as <code>@spool-lab/connector-*</code> (or your own scope).
        Read the source, fork it, publish your own.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Search, list, show, install, sync — every action works from <code>spool-daemon</code> on the
        terminal or from the app. Pick whichever fits the moment.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Spool Daemon runs alone — you don't need Spool to use it. Pair with Spool to make captures
        and sessions searchable from one box.
      </>
    ),
  },
]
        ...
        <div className="hero hero-daemon">
          <div>
            ...
            <h1 className="display">
              ...
              <br />
              <em>
                captures<span className="accent">.</span>
              </em>
            </h1>
            <p className="lede">
              Spool Daemon is a standalone app that quietly pulls your stars, bookmarks, saves and
              notes into a local SQLite database — searchable from its own UI, or paired with Spool.
              Plugins for the platforms you care about. Nothing leaves the machine.
            </p>
      ...
      <main className="wrap">
        ...
        <section className="sect" id="connectors">
          <div className="s-head">
            ...
            <div>
              <h2 className="s-title">
                One connector per <em>platform</em>
                <span className="accent">.</span>
              </h2>
              <p className="s-sub">
                Each connector is a small npm package. Install from the CLI or click{' '}
                <code>Install</code> to launch directly into Spool Daemon.
              </p>
            </div>
          </div>
          <div className="connector-grid">
            ...
            >
              ...
              <p className="c-desc">
                Implement the <code>Connector</code> interface, publish to npm, install from
                anywhere. Daemon handles scheduling, retries and dedupe.
              </p>
              <div className="c-foot">
                <span className="c-meta">
        ...
        <section className="sect">
          <div className="s-head">
            ...
            <div>
              <h2 className="s-title">
                How the daemon <em>works</em>
                <span className="accent">.</span>
              </h2>
              <p className="s-sub">
                Three things, in order. Pick connectors, let them sync, search the local index —
                from the app, from the CLI, or both.
              </p>
            </div>
          </div>
        ...
        <section className="sect" id="plugins">
          <div className="s-head">
            ...
            <div>
              <h2 className="s-title">
                Write your own <em>connector</em>
                <span className="accent">.</span>
              </h2>
              <p className="s-sub">
                Implement the{' '}
                ...
                backoff, dedupe and writes to the local index.
              </p>
            </div>
          </div>
          <div className="plugin-grid">
            <div className="plugin-prose plugin-prose-1">
              ...
              <p>
                ...
                <code>fetchPage()</code>. <code>caps.fetch</code> is the daemon's managed fetch
                (handles abort, system proxies, logging). Items emitted as{' '}
                <code>CapturedItem[]</code> get written to the local FTS5 index — no schema work for
                you.
              </p>
            </div>
            <div className="plugin-prose plugin-prose-2">
              ...
              <p>
                The daemon finds your connector through the <code>"spool"</code> block in{' '}
                <code>package.json</code> — id, platform, label, color, capabilities. Publish to npm
                and anyone can <code>spool-daemon install</code> it. To show up in the public
                directory, send a PR to <code>spool-lab/spool-daemon</code> with one entry.
              </p>
            </div>
