...
export const securityApi = {
  ...
  ): Promise<SessionWithFindingCounts[]> => window.spool.security.listSessionsWithFindings(filter),
  ...
  riskByCategory: (): Promise<RiskByCategoryRow[]> => window.spool.security.riskByCategory(),
  lastScanCompletedAt: (): Promise<string | null> => window.spool.security.lastScanCompletedAt(),
  ...
  getScanStatus: (): Promise<ScanStatus> => window.spool.security.getScanStatus(),
  ...
  undismissFinding: (findingId: number) => window.spool.security.undismissFinding(findingId),
  purgeFinding: (findingId: number) => window.spool.security.purgeFinding(findingId),
  purgeFindings: (findingIds: number[]) => window.spool.security.purgeFindings(findingIds),
  ...
  getPrefs: (): Promise<SecurityPreferences> => window.spool.security.getPrefs(),
  ...
  countAllowlistEntries: (): Promise<number> => window.spool.security.countAllowlistEntries(),
  ...
  listBackups: (): Promise<BackupFileInfo[]> => window.spool.security.listBackups(),
  deleteBackups: (names: string[]): Promise<DeleteBackupsResult> =>
    window.spool.security.deleteBackups({ names }),

  pfGetState: (): Promise<PfDownloadState> => window.spool.security.pfGetState(),
  pfDownloadStart: (): Promise<{ ok: boolean; reason?: string }> =>
    window.spool.security.pfDownloadStart(),
  pfDownloadCancel: (): Promise<{ ok: boolean }> => window.spool.security.pfDownloadCancel(),
  pfGetRuntimeInfo: (): Promise<PfRuntimeInfo | null> => window.spool.security.pfGetRuntimeInfo(),
  onPfState: (handler: (s: PfDownloadState) => void): (() => void) =>
    window.spool.security.onPfState(handler),

  getReadiness: (): Promise<SecurityReadiness> => window.spool.security.getReadiness(),
