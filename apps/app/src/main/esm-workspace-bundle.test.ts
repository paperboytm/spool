...
const IMPORT_ONLY_WORKSPACE_PACKAGES = [
  '@spool-lab/cli/hub',
...
describe('main-process ESM workspace bundles', () => {
  it('does not leave import-only packages behind as CommonJS requires', async () => {
    ...
    await execFileAsync('npx', ['electron-vite', 'build', '--outDir', OUT_DIR], {
      cwd: APP_ROOT,
    })

    const bundle = readFileSync(join(OUT_DIR, 'main', 'index.js'), 'utf8')
    for (const packageName of IMPORT_ONLY_WORKSPACE_PACKAGES) {
      const hasRuntimeRequire = bundle.includes(`require("${packageName}")`)
        || bundle.includes(`require('${packageName}')`)
      expect(
        hasRuntimeRequire,
