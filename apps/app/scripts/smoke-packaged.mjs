...
assert(
  existsSync(join(unpackedModules, 'acp-extension-claude', 'dist', 'index.js')),
...
writeFileSync(
  ...
  JSON.stringify({
    ...
    message: {
      role: 'user',
      content: `PACKAGED_SMOKE_SECRET ${secret}. My name is Harry Potter and my email is harry.potter@hogwarts.edu.`,
    },
...
try {
  ...
  const searchResults = await poll(async () => {
    const results = await window.evaluate(
      async () => globalThis.spool?.search('XYLOPHONE_CANARY_42', 5) ?? [],
    )
  ...
  for (const agentId of ['claude', 'codex', 'pi']) {
    ...
    const response = await window.evaluate(
      async (id) => globalThis.spool.aiSearch('packaged ACP smoke', id, []),
      agentId,
    )
    assert(
      response.ok && response.fullText?.includes('MOCK_ACP_RESPONSE_42'),
      `${agentId} ACP launch failed: ${response.error ?? 'empty response'}`,
    )
  }
  ...
  const sensitiveResults = await window.evaluate(async () =>
    globalThis.spool.search('PACKAGED_SMOKE_SECRET', 5),
  )
  ...
  await window.evaluate(
    async (findingId) => globalThis.spool.security.purgeFinding(findingId),
    apiKey.id,
  )
  const purgedSession = await window.evaluate(async () =>
    globalThis.spool.getSession('packaged-sensitive-session'),
  )
  const purgedText = purgedSession?.messages.map((message) => message.contentText).join('\n') ?? ''
  assert(
    !purgedText.includes(secret) && purgedText.includes('[redacted:'),
    'packaged purge did not mask the indexed message',
  )
  const postPurgeSearch = await window.evaluate(
    async (query) => globalThis.spool.search(query, 5),
    secret,
  )
  ...
  if (fullPrivacyFilter) {
    if (pfState.phase !== 'installed') {
      const download = await window.evaluate(async () =>
        globalThis.spool.security.pfDownloadStart(),
      )
    ...
    const pfScanStatus = await poll(
      ...
      'Privacy Filter scan completion',
      180_000,
    )
    assert(
      pfScanStatus.currentProfile.includes('pf@'),
      `scan profile did not enable Privacy Filter: ${pfScanStatus.currentProfile}`,
    )
  }
  assert(
    ['not-installed', 'installed'].includes(pfState.phase),
