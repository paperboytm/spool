...
test('window recovers after close → activate cycle', async () => {
  ...
  await expect(restoredAgain.locator('[data-testid="library-landing"]')).toBeVisible({
    timeout: 10000,
  })
})
...
async function closeAndAwaitDestroy(
  ...
  await app.evaluate(
    ({ BrowserWindow }) =>
      new Promise<void>((resolve) => {
        const tick = () =>
          BrowserWindow.getAllWindows().length === 0 ? resolve() : setTimeout(tick, 25)
        tick()
      }),
  )
}
