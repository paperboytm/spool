...
describe('PATCH /api/me/shares/:id (visibility)', () => {
  it('rejects unauthenticated requests with 401', async () => {
    ...
    const res = await invoke(
      visibilityPatch,
      patchReq(share.id, { visibility: 'profile-listed' }, null),
      env,
      { id: share.id },
    )
  ...
  it("404s another user's share without leaking its existence", async () => {
    ...
    const res = await invoke(
      visibilityPatch,
      patchReq(share.id, { visibility: 'profile-listed' }),
      env,
      { id: share.id },
    )
  ...
  it('404s an invalid slug before touching auth or DB', async () => {
    ...
    const res = await invoke(
      visibilityPatch,
      patchReq('!bad slug!', { visibility: 'unlisted' }),
      env,
      { id: '!bad slug!' },
    )
    expect(res.status).toBe(404)
  ...
  it('422s an unknown visibility value', async () => {
    const { env, share } = await setup()
    const res = await invoke(visibilityPatch, patchReq(share.id, { visibility: 'public' }), env, {
      id: share.id,
    })
  ...
  it('400s malformed json', async () => {
    const { env, share } = await setup()
    const res = await invoke(visibilityPatch, patchReq(share.id, '{not json'), env, {
      id: share.id,
    })
  ...
  it('allows unlisting even without a handle', async () => {
    ...
    const res = await invoke(visibilityPatch, patchReq(share.id, { visibility: 'unlisted' }), env, {
      id: share.id,
    })
  ...
  it("UPDATE is scoped to user_id: a passing ownership SELECT still can't mutate another user's row", async () => {
    ...
    const intercept = (sql: string) => {
      if (
        /^SELECT visibility, revoked_at FROM published_shares WHERE id=\? AND user_id=\?/i.test(sql)
      ) {
        return {
    ...
    const res = await invoke(
      visibilityPatch,
      patchReq(victim.id, { visibility: 'profile-listed' }),
      env,
      { id: victim.id },
    )
    expect(res.status).toBe(200)
  ...
  it('429s past the hourly per-user cap', async () => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const r = await invoke(visibilityPatch, patchReq(share.id, { visibility: flip }), env, {
        id: share.id,
      })
