...
async function shareWithSpoolFile(env: Env) {
  const record = await canonicalizeRecord(JSON.stringify({
    type: 'user', uuid: 'u-1', sessionId: 's',
    message: { role: 'user', content: 'hello' },
  }))
  const view = await canonicalizeRecord(JSON.stringify({ v: 1, index: [], files: [], outline: [], firstPrompt: 'hello', lastReply: '', diffstat: { files: 0, adds: 0, dels: 0 } }))
  const spoolFile = await canonicalizeRecord(JSON.stringify({
    version: 2,
  ...
  const pushRes = await invoke(pushPost, new Request(`https://hub.test/api/hub/v1/sessions/${SID}/push`, {
    method: 'POST', headers: auth, body: JSON.stringify(head),
  }), env, { sid: SID })
  ...
  const lines = [record, view, spoolFile]
    ...
    .join('\n')
  await invoke(batchPost, new Request('https://hub.test/api/hub/v1/objects/batch', {
    method: 'POST', headers: auth, body: lines,
  }), env)

  const headRes = await invoke(headPost, new Request(`https://hub.test/api/hub/v1/sessions/${SID}/head`, {
    method: 'POST', headers: auth, body: JSON.stringify(head),
  }), env, { sid: SID })
...
describe('hub spool-file attachment', () => {
  it('counts the spool file in missing, commits it on the head, and serves it back', async () => {
    ...
    const metaRes = await invoke(metaGet, new Request(`https://hub.test/api/hub/v1/sessions/${SID}`), env, { sid: SID })
    const meta = (await metaRes.json()) as { spoolFileOid: string | null; viewOid: string }
    expect(meta.spoolFileOid).toBe(head.spoolFileOid)

    const fileRes = await invoke(spoolFileGet, new Request(`https://hub.test/api/hub/v1/sessions/${SID}/spool-file`), env, { sid: SID })
    expect(fileRes.status).toBe(200)
    expect(fileRes.headers.get('content-type')).toBe('application/spool+json')
    expect(await fileRes.text()).toBe(spoolFile.data)

    const cached = await invoke(spoolFileGet, new Request(`https://hub.test/api/hub/v1/sessions/${SID}/spool-file`, {
  ...
  it('404s the spool-file endpoint when the share carries none', async () => {
    const env = makeEnv()
    const record = await canonicalizeRecord('{"type":"user","message":{"role":"user","content":"x"}}')
    const view = await canonicalizeRecord('{"v":1}')
    const head = {
      root: await sequenceRoot([record.oid]),
      count: 1,
      manifest: [record.oid],
      sig: null, cardJson: null, noteMd: null, lineageJson: null,
      viewOid: view.oid,
      // No spoolFileOid at all — older clients omit the field entirely.
    }
    await invoke(pushPost, new Request(`https://hub.test/api/hub/v1/sessions/${SID}/push`, {
      method: 'POST', headers: auth, body: JSON.stringify(head),
    }), env, { sid: SID })
    await invoke(batchPost, new Request('https://hub.test/api/hub/v1/objects/batch', {
