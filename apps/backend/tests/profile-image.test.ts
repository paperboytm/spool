...
function buildPngHeader(
  width: number,
  height: number,
  extraChunks: Uint8Array = new Uint8Array(),
): Uint8Array {
...
function buildExifChunk(): Uint8Array {
  ...
  buf[8 + data.length] = 0
  buf[9 + data.length] = 0
  buf[10 + data.length] = 0
  buf[11 + data.length] = 0
  return buf
}
...
describe('stripMetadata (JPEG)', () => {
  ...
  it('strips APP13 (Photoshop / IPTC)', () => {
    ...
    const photoshop = new Uint8Array(
      Array.from('Photoshop 3.0\0iptcdata').map((c) => c.charCodeAt(0)),
    )
  ...
  it('preserves APP0 (JFIF — structural)', () => {
    const jfif = new Uint8Array(
      Array.from('JFIF\0\x01\x02\x00\x00\x60\x00\x60\x00\x00').map((c) => c.charCodeAt(0)),
    )
  ...
  it('preserves APP14 (Adobe colour-space flag)', () => {
    const adobe = new Uint8Array(
      Array.from('Adobe\0\x64\x80\x00\x00\x00\x00').map((c) => c.charCodeAt(0)),
    )
