...
import { listUserFollowing, resolveUserFollowTarget } from '../../../../src/social/users'
...
export const onRequestGet: PagesFunction<Env, Params> = async (ctx) => {
  try {
    ...
    const page = await listUserFollowing(
      ctx.env.DB,
      target,
      await parseSocialListOptions(ctx.request, ['user-following', target.id]),
    )
    return socialOk({ following: page.rows, next_cursor: page.nextCursor })
  } catch (error) {
