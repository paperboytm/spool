...
describe('composeSessionDiff', () => {
  it('composes repeated edits into one per-file net diff with record mapping', () => {
    ...
    const events: EditEvent[] = [
      ...
      },
      {
        ...
        before: afterFirst,
        after: afterSecond,
        replacements: [{ oldText: 'TWO', newText: 'SECOND' }],
      },
      {
        provider: 'claude',
        tool: 'Edit',
        recordIndex: 10,
        resultRecordIndex: 11,
        path: 'src/a.ts',
        before: afterSecond,
        after: final,
        replacements: [{ oldText: 'nine', newText: 'NINE' }],
      },
    ]
  ...
  it('composes separate and overlapping patch fragments without full file snapshots', () => {
    ...
    expect(file?.hunks.flatMap((hunk) => hunk.recordIndices)).toEqual(
      expect.arrayContaining([2, 6, 10]),
    )
  })
  ...
  it('orders Codex fragments by hunk coordinates and preserves wire line positions', () => {
    ...
    expect(file?.hunks.map((hunk) => [hunk.oldStart, hunk.newStart])).toEqual([
      [10, 10],
      [100, 100],
    ])
    expect(
