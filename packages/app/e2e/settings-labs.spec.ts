...
test('Security ships on by default: sidebar entry present, no Labs opt-in row', async () => {
  const { window } = ctx
  await waitForSync(window)

  // No agents.json opt-in is seeded by launchApp — the sidebar entry
  // being present proves Security no longer needs one.
  await expect(window.locator('[data-testid="sidebar-security"]')).toBeVisible()

  await openLabs(window)
  await expect(window.locator('[data-testid="labs-row-security"]')).toHaveCount(0)
  await window.keyboard.press('Escape')
})
