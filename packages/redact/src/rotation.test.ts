...
describe('rotationUrlForVendor', () => {
  it('maps verified vendors to their exact official rotation URL', () => {
    expect(rotationUrlForVendor('AWS')).toBe(
    ...
    expect(rotationUrlForVendor('GitLab')).toBe(
...
describe('rotationUrlForToken', () => {
  it('resolves vendor from a token prefix then maps to URL', () => {
    ...
    expect(rotationUrlForToken('sk-ant-api03-xyz')).toBe(
      'https://console.anthropic.com/settings/keys',
    )
    expect(rotationUrlForToken('AKIAIOSFODNN7EXAMPLE')).toBe(
      'https://console.aws.amazon.com/iam/home#/security_credentials',
    )
    expect(rotationUrlForToken('sk_live_' + 'x'.repeat(24))).toBe(
