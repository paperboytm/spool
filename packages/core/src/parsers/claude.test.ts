  ...
  it('uses first user message as title by default', () => {
    const fp = writeTmpSession([
      ...
      baseRecord({
        type: 'assistant',
        uuid: 'a1',
        message: { role: 'assistant', content: 'Sure', model: 'claude-opus-4-6' },
      }),
    ])
  ...
  it('skips noise from local-command-caveat / command-name / command-args slash invocations and uses real query as title', () => {
    ...
    const fp = writeTmpSession([
      ...
      baseRecord({
        ...
        message: {
          role: 'user',
          content: '<local-command-stdout>Set model to claude-opus-4-7</local-command-stdout>',
        },
      }),
      baseRecord({
        type: 'user',
        uuid: 'u4',
        message: { role: 'user', content: 'what did I do today?' },
      }),
      baseRecord({
        type: 'assistant',
        uuid: 'a1',
        message: { role: 'assistant', content: 'You did things.', model: 'claude-opus-4-6' },
      }),
    ])
    const result = parseClaudeSession(fp)
    expect(result?.title).toBe('what did I do today?')
    if (result?.kind === 'parsed') {
      const userTexts = result.session.messages
  ...
  it('preserves bare <command-args> content when not part of a slash-command triplet', () => {
    ...
    const fp = writeTmpSession([
      ...
      baseRecord({
        type: 'assistant',
        uuid: 'a1',
        message: { role: 'assistant', content: 'It means...', model: 'claude-opus-4-6' },
      }),
    ])
    const result = parseClaudeSession(fp)
