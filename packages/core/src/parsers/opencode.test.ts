...
describe('OpenCode parser', () => {
  ...
  it('folds grandchild subagent sessions into the root parent', () => {
    ...
    expect(loadOpenCodeSession(makeOpenCodeSessionFilePath(dbPath, 'ses_grandchild'))).toEqual({
      kind: 'filtered',
    })
  })
  ...
  it('does not surface a subagent whose parent row is missing (orphan stays folded-or-hidden)', () => {
    ...
    expect(listOpenCodeSessionFilePaths(dbPath)).toEqual([])
    expect(loadOpenCodeSession(makeOpenCodeSessionFilePath(dbPath, 'ses_orphan'))).toEqual({
      kind: 'filtered',
    })
  })
  ...
  it('keeps children of an archived parent hidden', () => {
    ...
    try {
      ...
      insertBareSession(db, {
        id: 'ses_arch_child',
        parentId: 'ses_arch_parent',
        agent: 'explore',
        title: 'Child of archived',
      })
      insertTextMessage(db, {
        sessionId: 'ses_arch_child',
        messageId: 'msg_ac',
        role: 'user',
        text: 'hidden work',
        at: start + 1000,
    ...
    expect(listOpenCodeSessionFilePaths(dbPath)).toEqual([])
    expect(loadOpenCodeSession(makeOpenCodeSessionFilePath(dbPath, 'ses_arch_child'))).toEqual({
      kind: 'filtered',
    })
  })
  ...
  it('does not hang and produces no roots on a self-referential / cyclic parent_id', () => {
    ...
    try {
      ...
      insertBareSession(db, {
        id: 'ses_cycle_a',
        parentId: 'ses_cycle_b',
        agent: 'build',
        title: 'A',
      })
      insertBareSession(db, {
        id: 'ses_cycle_b',
        parentId: 'ses_cycle_a',
        agent: 'build',
        title: 'B',
      })
      insertTextMessage(db, {
        sessionId: 'ses_cycle_a',
        messageId: 'msg_a',
        role: 'user',
        text: 'a',
    ...
    expect(listOpenCodeSessionFilePaths(dbPath)).toEqual([])
    expect(loadOpenCodeSession(makeOpenCodeSessionFilePath(dbPath, 'ses_cycle_a'))).toEqual({
      kind: 'filtered',
    })
  })
  ...
  it('maps WAL/SHM sidecar paths back to the main database file', () => {
    expect(normalizeOpenCodeWatchPath('/data/opencode/opencode.db-wal')).toBe(
      '/data/opencode/opencode.db',
    )
    expect(normalizeOpenCodeWatchPath('/data/opencode/opencode.db-shm')).toBe(
      '/data/opencode/opencode.db',
    )
    expect(normalizeOpenCodeWatchPath('/data/opencode/opencode.db-journal')).toBe(
      '/data/opencode/opencode.db',
    )
    expect(normalizeOpenCodeWatchPath('/data/opencode/opencode.db')).toBe(
      '/data/opencode/opencode.db',
    )
