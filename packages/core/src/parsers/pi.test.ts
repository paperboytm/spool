...
function makeSessionFile(lines: unknown[], fileName = '2026-04-02T09-05-13-662Z_f41a7803-b075-4b88-8d74-f46a3a06f67d.jsonl'): string {
...
function userMessage(id: string, parentId: string | null, text: string, timestamp = '2026-04-02T09:26:58.586Z'): unknown {
  return {
    type: 'message',
    id,
    parentId,
...
describe('parsePiSession', () => {
  it('parses a pi v3 session log with header, roles, tools, and model', () => {
    const filePath = makeSessionFile([
      HEADER,
      { type: 'thinking_level_change', id: '22ea80b9', parentId: null, timestamp: '2026-04-02T09:05:13.662Z', thinkingLevel: 'medium' },
      userMessage('e620d41e', '22ea80b9', 'Check the current changes with the review skill'),
      {
        ...
        message: {
          ...
          content: [
            ...
            { type: 'toolCall', id: 'toolu_01', name: 'read', arguments: { path: '/tmp/SKILL.md' } },
  ...
  it('takes the model from model_change when no assistant message names one', () => {
    const filePath = makeSessionFile([
      HEADER,
      { type: 'model_change', id: '2b5773cc', parentId: null, timestamp: '2026-04-02T09:05:13.672Z', provider: 'anthropic', modelId: 'claude-opus-4-8' },
  ...
  it('survives malformed lines and string content payloads', () => {
    const dir = mkdtempSync(join(tmpdir(), 'spool-pi-parser-'))
    tempDirs.push(dir)
    const filePath = join(dir, '2026-04-02T09-05-13-662Z_f41a7803-b075-4b88-8d74-f46a3a06f67d.jsonl')
    writeFileSync(filePath, [
      JSON.stringify(HEADER),
      '{broken json',
      JSON.stringify({ type: 'message', id: 'm1', parentId: null, timestamp: '2026-04-02T09:26:58.586Z', message: { role: 'user', content: 'plain string prompt' } }),
    ].join('\n') + '\n')
