...
- **Header:** Compact and persistent. Wordmark on the left; `Explore`, `Docs`, and search in the primary navigation; `Publish` and account state on the right. `Publish` uses the Paperboy blue accent. Signed-out visitors see `Sign in`; signed-in users see Team access and their account avatar. Product, account, and publishing-guide pages preserve this information architecture so entering a workflow never strands the user. On mobile, navigation may collapse into a menu, but `Explore`, `Docs`, `Publish`, and the relevant account action remain available with 44px targets.
