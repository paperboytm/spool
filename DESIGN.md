...
- **Projects:** A Project is a durable metadata home for related Sessions, owned by exactly one User or Team. It stores a name, URL-safe slug, short context-first description, and optional GitHub URL; it stores no repository content. Canonical identity follows the existing handle namespace as `/@handle/project-slug`. Personal Project metadata is public under its owner’s Profile. A Team Project gains a bounded public projection only while it contains at least one live Public Session; that projection exposes only its metadata, owner namespace, Public Sessions, Stars, and public stargazers. A Team-owned Project may therefore be published at `@team/project` while Team-only and Link-only Sessions, membership, management, and Watch state remain hidden. A Team Project with no live Public Session still resolves only for current members under `private, no-store`. Project ownership never replaces Session authorship: a Team owns the Project and hosted Session while the individual member remains the named author.
- **Profile:** User or Team identity and recurring topics come first, followed by Public Projects and Public Sessions. A person can be Followed; a Team cannot. Follower, following, Star, and Watch counts support useful Projects and Sessions rather than becoming the hero.
...
- **Implementation contract:** Tailwind CSS v4's CSS-first `@theme` namespace is the only component-facing design-token interface. Runtime light/dark values may remain CSS custom properties, but pages and shared components consume them through semantic Tailwind tokens such as `background`, `surface`, `foreground`, `border`, `accent`, status, source, spacing, type, radius, control, motion, and shadow tokens. Do not add page-local palette aliases, duplicate a light/dark Tailwind palette, or use Tailwind's default red/green/gray colors for product semantics.
...
- **Header:** Avatar, display name, `@handle`, short bio, recurring topics/agents, and restrained Project, Session, follower, and following counts. Another signed-in user gets one `Follow` / `Following` control; self and Team profiles do not.
- **Body:** Public Projects and Public Sessions only. Link-only and Team-only Sessions never affect visible counts or empty-state wording. `Stars`, `Followers`, and `Following` are route-addressable profile views; Watch state is never public.
- **Empty state:** “No public sessions yet.” Do not imply that the author has no local or Link-only work.

### Project social

- **Star:** A public bookmark and endorsement of a Project with at least one live Public Session. Star count and stargazers are public; the signed-in viewer may add or remove one idempotently. Stars disappear when the Project loses its final Public Session and do not affect `Top` ranking in the first release.
- **Watch:** A private subscription that puts a Project in the signed-in viewer's `Watching` Project scope. The UI says exactly that; it does not promise notifications until a notification system exists. Watcher identities are never public.
- **Follow:** A public directed relation between people. A person cannot follow themself, and a Team is not a person. Follower/following lists are public and lead back to Profiles and their readable work.
- **Team boundary:** Public Team Projects use the same Star and Watch controls as personal public Projects. A Team Project without a public projection may still be Watched by a current member, but its social response is `private, no-store`, outsiders receive the same unavailable response as an unknown Project, and membership removal clears that private relation.
- **Consumption:** Signed-in Project navigation includes `Starred` and `Watching` scopes. Project cards may show a quiet Star count; Watch belongs on the Project header and private scope, not as a second vanity metric on every row.
...
- **CLI:** `spool share` and `spool subscribe` resolve the exact local Project of the selected Session, then require an explicit Hub Project binding. A personally authenticated member may select `team-handle/project-slug`; that Project determines Team ownership while visibility remains an independent explicit choice (`Public`, `Link-only`, or `Team · {name}`). Interactive use may select or create a personal or writable Team Project; non-interactive use fails closed without `--project` or an existing binding, and `--yes` never chooses a Project or widens visibility. Bindings are account-, tenant-, Hub-, and local-identity-specific. Re-sharing preserves the existing remote Project unless the author performs an explicit management move.
...
- **Team target:** Team ownership is an explicit optional target, never a sticky hidden default. Selecting a Team Project always states that the Team owns the resulting Session while the member remains its author. Visibility is confirmed separately: `Team · {name}` means current members only, while `Public` and `Link-only` keep their normal audience under the Team-owned Project.
...
- A Watch or Follow control without a route where the saved relationship is useful
- Ambiguous visibility controls, icon-only publish actions, or language that calls a Link-only URL private
- Page-local palette aliases, duplicated light/dark Tailwind themes, or broad descendant selectors that can restyle shared primitives
...
| 2026-07-26 | Team Projects can publish a public projection   | Supersedes the member-only portion of the earlier Project decision. A Team may own a canonical `@team/project` containing Public, Link-only, and Team-only Sessions. Only metadata and live Public Sessions enter the public namespace; authorship remains attributed to the contributing member, and every private tenant surface keeps its membership gate.                                                     |
| 2026-07-26 | Projects and people form a useful social graph  | Public Projects support Stars and private Watching; people support public Follow relationships. Starred, Watching, Followers, and Following each have a real reading/navigation surface, watcher identities remain private, and none of these counts changes global ranking in the first release.                                                                                                                 |
| 2026-07-26 | Tailwind theme tokens are the styling interface | Tailwind v4 `@theme` semantic tokens are the single component-facing design-system vocabulary. Light/dark runtime variables remain implementation details; duplicate palettes and page-local aliases have been removed without changing the documented Void Index values.                                                                                                                                         |
